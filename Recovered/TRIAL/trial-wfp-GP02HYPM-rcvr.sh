#!/bin/bash
#Ingest.ctdmo-ghqr_recovered          /omc_data/whoi/OMC/GP02HYPM/R00001/mc/raw/SBE37*.hex         GP02HYPM-RIS01-01-CTDMOG000
...
#Pick up the files:
DAT_A_12936_files=$(ls $DAT_A_12936 -print|head -250)
DAT_C_12936_files=$(ls $DAT_C_12936 -print|head -250)
DAT_E_12936_files=$(ls $DAT_E_12936 -print|head -250)

DAT_A_12774_files=$(ls $DAT_A_12774 -print|head -250)
DAT_C_12774_files=$(ls $DAT_C_12774 -print|head -250)
DAT_E_12774_files=$(ls $DAT_E_12774 -print|head -250)

#DAT_CTD_files=$(ls $DAT_CTD -print)
DAT_STA_files=$(ls $DAT_STA -print|head -250)
HEX_SBE_files=$(ls $HEX_SBE -print|head -250)
...
do
    if  grep --quiet $files $EDEXLOGFILES; then
        ...
        ls -l $files
        echo $INGS Ingest.vel3d-l-wfp_recovered $files GP02HYPM-WFP03-05-VEL3DL000 recovered
             $INGS Ingest.vel3d-l-wfp_recovered $files GP02HYPM-WFP03-05-VEL3DL000 recovered
        sleep 3
    fi
done
...
do
    if  grep --quiet $files $EDEXLOGFILES; then
        ...
        ls -l $files
        echo $INGS Ingest.ctdpf-ckl-wfp_recovered $files GP02HYPM-WFP03-04-CTDPFL000 recovered
             $INGS Ingest.ctdpf-ckl-wfp_recovered $files GP02HYPM-WFP03-04-CTDPFL000 recovered
        sleep 3
    fi
done
...
do
    if  grep --quiet $files $EDEXLOGFILES; then
        ...
        ls -l $files
        echo $INGS Ingest.dosta-ln-wfp_recovered $files GP02HYPM-WFP03-03-DOSTAL000 recovered
             $INGS Ingest.dosta-ln-wfp_recovered $files GP02HYPM-WFP03-03-DOSTAL000 recovered
        sleep 3
        echo $INGS Ingest.flord-l-wfp_recovered $files GP02HYPM-WFP03-01-FLORDL000 recovered
             $INGS Ingest.flord-l-wfp_recovered $files GP02HYPM-WFP03-01-FLORDL000 recovered
        sleep 3
        echo $INGS Ingest.wfp-eng-stc-imodem_recovered $files GP02HYPM-WFP03-00-ENG000000 recovered
             $INGS Ingest.wfp-eng-stc-imodem_recovered $files GP02HYPM-WFP03-00-ENG000000 recovered
        sleep 3
    fi
done
