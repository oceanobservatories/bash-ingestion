...
date 
echo "start cp-glider-rcvr-command.sh" 
...
do
    ...
    count=$((count+1))
    echo $count
    if [ "$count" -gt " $(expr $total / 400)" ];then
        break
    fi
done
...
do
    if  grep --quiet $file $EDEXLOGFILES; then
        ...
        #$INGS Ingest.adcpa-m-glider_recovered $file CP05MOAS-GL001-02-ADCPAM000 recovered
    fi
    sleep 3
